        ...
        <tbody>
          {entries.map((entry) => (
            <tr key={entry.id} className="border-b border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
              ...
              <td className={`py-3 text-right font-medium tabular-nums ${entry.direction === 'income' ? 'text-emerald-600' : 'text-zinc-900 dark:text-zinc-100'}`}>
                {entry.direction === 'income' ? '+' : '-'} R$ {(entry.amountCents / 100).toFixed(2)}
              </td>
            </tr>
          ))}
        </tbody>
